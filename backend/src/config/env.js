...
const env = {
  ...
  DB_HOST: process.env.DB_HOST || process.env.DB_SERVER || 'localhost',
  DB_PORT: parseInt(process.env.DB_PORT || '5432', 10) || 5432,
  DB_NAME: process.env.DB_NAME || process.env.DB_DATABASE || 'bulkapply',
  DB_USER: process.env.DB_USER || 'postgres',
  DB_PASSWORD: process.env.DB_PASSWORD || '',
  DB_URL: process.env.DIRECT_URL || process.env.DATABASE_URL || process.env.DB_URL || '',
...
if (env.NODE_ENV === 'production') {
  const required = ['JWT_SECRET', 'DB_PASSWORD', 'GROQ_API_KEY', 'SMTP_USER', 'SMTP_PASS'];
